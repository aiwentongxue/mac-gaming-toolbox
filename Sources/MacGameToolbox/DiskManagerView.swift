...
struct DiskManagerView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            HStack {
                VStack(alignment: .leading) {
                    ...
                    Text(tr("最多选择 3 个卷；启动磁盘已自动排除。", "Select up to three volumes. The startup disk is excluded automatically.")).foregroundStyle(.secondary)
                }
                Spacer()
            ...
            HStack {
                Text(tr("已选择 \(model.selectedDiskIDs.count)/3", "Selected \(model.selectedDiskIDs.count)/3")).foregroundStyle(.secondary)
                Spacer()
    ...
    private func selectedBinding(_ id: String) -> Binding<Bool> {
        Binding {
            ...
            if selected {
                if model.selectedDiskIDs.count < 3 { model.selectedDiskIDs.insert(id) }
            } else {
