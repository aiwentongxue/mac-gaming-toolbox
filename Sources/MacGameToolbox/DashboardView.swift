...
struct DashboardView: View {
    ...
    @ViewBuilder private var featureCards: some View {
        ...
        FeatureCard(icon: "hand.point.up.left.fill", title: tr("连接鼠标时禁用 Mac 触控板", "Disable Mac Trackpad with a Mouse"), subtitle: tr("连接鼠标或无线触控板时忽略内建触控板，防止游戏中误触", "Ignore the built-in trackpad while a mouse or wireless trackpad is connected to prevent accidental input while gaming")) {
            Toggle(
